...
----------------------{ Get Name Bot }-----------------------------
Bot_Name = redis:get(lana..":NameBot:")
...
var = 'مطور البوت 👨🏽‍💻' 
elseif redis:sismember(lana..':KARA_BOT:'..ChatID,UserID) then
var = ' المنشىء الاساسي👷🏽' 
...
EnterCmd = true
local NumberFlood = tonumber(redis:get(lana..':Flood_Spam:'..msg.sender_user_id_..':msgs') or 0)
if NumberFlood > 3 then 
redis:setex(lana..':Flood_Spam:'..msg.sender_user_id_..':Stoped',10,true)
EnterCmd = false
elseif redis:get(lana..':Flood_Spam:'..msg.sender_user_id_..':Stoped') then
EnterCmd = false
else
redis:setex(lana..':Flood_Spam:'..msg.sender_user_id_..':msgs',3,NumberFlood+1)
end
return EnterCmd
end
...
redis:hset(lana..'username:'..data.id_,'username',ResolveUser(data))
redis:sadd(lana..'admins:'..msg.chat_id_,data.id_)
end,nil)
end
end
if NumAdmin == 0 then 
...
if redis:get(lana..'group:add'..msg.chat_id_) then  return '🎗*│*المجموعه بالتأكيد ✓️ تم تفعيلها' end
...
redis:setex(lana..":ismember:"..msg.sender_user_id_,1500,true)
...
local lock_service = redis:get(lana..'lock_service')
...
GetUserID(v.user_id_,function(arg,data)
redis:hset(lana..'username:'..data.id_, 'username', ResolveUser(data))
redis:sadd(lana..':KARA_BOT:'..msg.chat_id_,data.id_)
end)
...
redis:mset(
lana..'group:add'..msg.chat_id_,true,
lana..'lock_link'..msg.chat_id_,true, 
lana..'lock_id'..msg.chat_id_,true,
lana..'lock_spam'..msg.chat_id_,true,
lana..'lock_webpage'..msg.chat_id_,true,
lana..'lock_markdown'..msg.chat_id_,true,
lana..'lock_flood'..msg.chat_id_,true,
...
lana..'mute_keyboard'..msg.chat_id_,true,
lana..'mute_game'..msg.chat_id_,true,
lana..'mute_inline'..msg.chat_id_,true,
lana..'lock_username'..msg.chat_id_,true,
lana..'num_msg_lana'..msg.chat_id_,5, 
lana..'replay'..msg.chat_id_,true
)

redis:sadd(lana..'group:ids',msg.chat_id_) 
redis:sadd(lana..'mtwr_count'..msg.sender_user_id_,msg.chat_id_)

GroupTitle(msg.chat_id_,function(arg,data)
local NameGroup = data.title_
redis:set(lana..'group:name'..msg.chat_id_,NameGroup)
...
redis:set(lana..'linkGroup'..msg.chat_id_,Gp_Link)
...
if not redis:get(lana..'group:add'..msg.chat_id_) then return '🛠*│* المجموعه بالتأكيد ✓️ تم تعطيلها' end  
...
if redis:sismember(lana..':KARA_BOT:'..ChatID,UserID) then 
return SendMention(ChatID,UserID,MsgID,'🙍🏻‍♂╿العضو » ❪ '..USERNAME..' ❫\n🎫│الايدي » ❪ '..UserID..' ❫\n💯╽انه بالتأكيد منشىء اساسي \n✓️',17,USERCAR) 
end
redis:hset(lana..'username:'..UserID,'username',Resolv)
redis:sadd(lana..':KARA_BOT:'..ChatID,UserID)
return SendMention(ChatID,UserID,MsgID,'🙍🏻‍♂╿العضو » ❪ '..USERNAME..' ❫\n🎫│الايدي » ❪ '..UserID..' ❫\n💯╽تمت ترقيته ليصبح منشىء اساسي في البوت \n✓️',17,USERCAR) 
end

if cmd == "remkara" then
if not redis:sismember(lana..':KARA_BOT:'..ChatID,UserID) then
return SendMention(ChatID,UserID,MsgID,'🙍🏻‍♂╿العضو » ❪ '..USERNAME..' ❫\n🎫│الايدي » ❪ '..UserID..' ❫\n💯╽انه بالتأكيد ليس منشىء اساسي\n✓️',17,USERCAR) 
end
redis:srem(lana..':KARA_BOT:'..ChatID,UserID)
...
if cmd == "unban" then
if not redis:sismember(lana..'banned:'..ChatID,UserID) then 
...
if redis:sismember(lana..'is_silent_users:'..ChatID,UserID) then 
return SendMention(ChatID,UserID,MsgID,'🙍🏻‍♂╿العضو » ❪ '..USERNAME..' ❫\n🎫│الايدي » ❪ '..UserID..' ❫\n💯╽تم بالتأكيد كتمه  \n✓️',17,USERCAR) 
end
redis:hset(lana..'username:'..UserID, 'username', Resolv)
...
if not redis:sismember(lana..'is_silent_users:'..ChatID,UserID) then 
...
if redis:sismember(lana..'banned:'..ChatID,UserID) then 
return sendMsg(ChatID,MsgID,'🙍🏻‍♂*╿*العضو » ❪ '..UserName..' ❫\n🎫*│*الايدي » ❪ `'..UserID..'` ❫\n💯*╽*تم بالتأكيد حظره \n✓️') 
end
kick_user(UserID, ChatID,function(arg,data)
if data.ID == "Error" and data.code_ == 400 then
...
if cmd == "dn_sudo" then
if not redis:sismember(lana..':SUDO_BOT:',UserID) then return sendMsg(ChatID,MsgID,'🙍🏻‍♂*╿*العضو » '..USERNAME..' \n🎫*│* الايدي » (`'..UserID..'`)\n💯*╽*انه بالتأكيد ليس مطور \n✓') end
redis:srem(lana..':SUDO_BOT:',UserID) 
return SendMention(ChatID,UserID,MsgID,'🙍🏻‍♂╿العضو » ❪ '..USERNAME..' ❫\n🎫│الايدي » ❪ '..UserID..' ❫\n💯╽تم تنزيله من المطورين \n✓️',17,USERCAR) 
end
...
if redis:get(lana..'welcome:get'..msg.chat_id_) then welcomex = "❪ مفعل ❫" else welcomex = "❪ معطل ❫" end
if redis:get(lana..'lock_id'..msg.chat_id_) then lock_idx = "❪ مفعل ❫" else lock_idx = "❪ معطل ❫" end
if redis:get(lana..'replay'..msg.chat_id_)  then replayx  = "❪ مفعل ❫" else replayx  = "❪ معطل ❫" end
if redis:get(lana..'lock_woring'..msg.chat_id_) then lock_woringx = "❪ مفعل ❫" else lock_woringx  = "❪ معطل ❫" end
...
.."\n⃣│التاك «» "..(redis:get(lana..'lock_tag'..msg.chat_id_) or 'false')
...
.."\n🏌🏻│بالطرد البوتات «» "..(redis:get(lana..'lock_bots_by_kick'..msg.chat_id_) or 'false')
.."\n🤖│البوتات «» "..(redis:get(lana..'lock_bots'..msg.chat_id_) or 'false')
.."\n➕│عدد التكرار «» "..(redis:get(lana..'num_msg_lana'..msg.chat_id_) or 'false')
...
.."\n🎙│البصمات «» "..(redis:get(lana..'mute_audio'..msg.chat_id_) or 'false')
.."\n🔉│الصوت «» "..(redis:get(lana..'mute_voice'..msg.chat_id_) or 'false')
